...
右下角常驻一只小飞宠，气泡显示令牌余额与今日已用；点击桌宠展开明细面板（剩余 / 总额度 / 今日 Token / 最近调用记录），可拖动、右键打开菜单（立即刷新 / 明细面板 / 设置 / 退出桌宠）。

- **布局**：卡片与气泡水平居中，桌宠固定在右下角，底边位置不变，展开/收起都不跳动。明细卡片按固定行高排版（头部 / 额度卡 / Token 条 / 状态行 / 三行调用记录 / 脚注），窗口高度由卡片内容反推，不会再出现内容被裁掉或行距错乱。
- **数字简写**：Token 与额度大数一律简写 —— `K` 千、`M` 百万、`B` 十亿，例如今日输入 `1,159,134,252` 显示为 **1.16B**、`54,818` 显示为 **54.8K**；金额保持 `¥267.86` 这类两位小数，不再出现一长串数字。
...
1. **应用内开关（推荐）**：打开 ConfigPilot → 左侧「设置」页 → 「余额监控桌宠」分组 → 打开「显示余额桌宠」开关即可（已配 Codex/Claude 的话直接就能看余额）；点「打开设置」可切换来源、改轮询间隔与金额换算。开关状态持久化，下次启动自动恢复；桌宠右键「退出桌宠」会关掉开关（同一进程内再打开开关可复用同一窗口）。
